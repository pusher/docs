...
# Publish to a specific User: Flutter
...
##### What is a `BeamsAuthProvider`?

When the Beams SDK tries to authenticate a device it will verify the user's identity by requesting a Beams Token from your application backend. The SDK will do this using the `BeamsAuthProvider` that you pass to `setUserId`. The `BeamsAuthProvider` must have access to the data needed to authenticate with your application backend. It gets this data using an `AuthData` callback.
